...
void aBroker::broker_thread()
{
    ...
    assert(this);
    m_isRunning = true;
    int64_t heartbeat_at = current_time() + HEARTBEAT_INTERVAL;
    while(m_isRunning)
    {
        ...
        if (items[0].revents & ZMQ_POLLIN)
        {
            ...
            //HEARTBEAT_MESSAGE
            if (msgType == HEARTBEAT_PONG && !heartbeat_message(msg))
            {
                Print(DBG_LEVEL_ERROR,"broker_thread: Couldn't parse heartbeat message.\n");
                delete msg;
                continue;
            }
        }
        ...
        if (current_time() > heartbeat_at)
        {
            ...
            heartBeat_msg->SetMessageType(HEARTBEAT_PING);
            m_mutex.lock();
            for(std::map<std::string, aSubscriber*>::iterator it = m_subscribers.begin();
                it != m_subscribers.end(); ++it)
            {
                SendMessage(heartBeat_msg, it->second->GetID());
            }
            for(std::map<std::string, aPublisher*>::iterator it = m_publishers.begin();
                it != m_publishers.end(); ++it)
            {
                SendMessage(heartBeat_msg, it->second->GetID());
            }
            m_mutex.unlock();
            heartbeat_at = current_time() + HEARTBEAT_INTERVAL;
...
void aBroker::purge_clients()
{
    int64_t clock = current_time();
    m_mutex.lock();
    for(std::map<std::string, aSubscriber*>::iterator it = m_subscribers.begin();
        it != m_subscribers.end(); )
    {
        if (clock > it->second->m_expiry)
        {
            Print(DBG_LEVEL_DEBUG, "Deleting subscriber with name '%s'\n", it->first.c_str());
            delete it->second;
        ...
        it != m_publishers.end(); )
    {
        if (clock > it->second->m_expiry)
        {
            Print(DBG_LEVEL_DEBUG, "Deleting publisher with name '%s'\n", it->first.c_str());
            delete it->second;
            it = m_publishers.erase(it);
        }
        else
            ++it;
    }
...
bool aBroker::SendMessage(DlgMessage *msg, const std::string &identity)
{
...
bool aBroker::AddSubscriber(const char *id)
{
    ...
    m_mutex.lock();
    m_subscribers[from] = new aSubscriber(id, current_time() + HEARTBEAT_INTERVAL * HEARTBEAT_LIVENESS);
    m_mutex.unlock();
    return true;
}
...
bool aBroker::AddPublisher(const char* id)
{
    ...
    m_mutex.lock();
    m_publishers[pub] = new aPublisher(id, current_time() + HEARTBEAT_INTERVAL * HEARTBEAT_LIVENESS);
    m_mutex.unlock();
    return true;
}
...
bool aBroker::heartbeat_message(DlgMessage *msg)
{
    std::string identity;
    if (!msg->GetIdentity(identity))
    {
        Print(DBG_LEVEL_ERROR,"aBroker::heartbeat_message: Couldn't get identity.\n");
        return false;
    }
    Print(DBG_LEVEL_DEBUG, "HeartBeat message was received from '%s'\n", identity.c_str());
    if (m_subscribers.find(identity) != m_subscribers.end())
    {
        m_subscribers.find(identity)->second->m_expiry =
                current_time() + HEARTBEAT_INTERVAL * HEARTBEAT_LIVENESS;
    }
    else if(m_publishers.find(identity) != m_publishers.end())
    {
        m_publishers.find(identity)->second->m_expiry =
                current_time() + HEARTBEAT_INTERVAL * HEARTBEAT_LIVENESS;
    }
    else
    {
        Print(DBG_LEVEL_ERROR,"aBroker::heartbeat_message: "
                              "Couldn't find PUB or SUB with identity '%s'.\n", identity.c_str());
        return false;
    }
    delete msg;
    return true;
...
DlgServer::DlgServer() : m_router(nullptr), m_main_thread(nullptr)
{
    try
    {
        ...
        Print(DBG_LEVEL_DEBUG, "Trying to bind to '%s'\n",endpoint);
