...
DlgMessage::DlgMessage(const std::string& name, const std::string& from, const std::string& to,
                       MessageType msgType, const std::string& body) : message_array_t()
{
    PushBack(name.c_str());
    PushBack(from.c_str());
    PushBack(to.c_str());
...
bool DlgMessage::GetMessageType(MessageType& msgType)
{
    const int idx = 3;
    ...
    if(s != sizeof(uint32_t))
        return false;
    msgType = *((MessageType*)m_data[idx].data()+1);
    return true;
}
...
bool DlgMessage::SetMessageType(MessageType msgType)
{
    _type = msgType;
